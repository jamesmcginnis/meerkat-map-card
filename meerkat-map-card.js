...
const MM_POPUP_KEYFRAMES = `@keyframes mmFadeIn  { from{opacity:0} to{opacity:1} } @keyframes mmSlideUp { from{transform:translateY(20px) scale(0.97);opacity:0} to{transform:none;opacity:1} } @keyframes mmPulse   { 0%,100%{transform:scale(1);opacity:1} 50%{transform:scale(1.18);opacity:0.7} }`;
...
this.*renderPOILayer(cat, elements);
...
try { localStorage.setItem(cacheKey, JSON.stringify({ ts: Date.now(), elements })); } catch (*) {}
